...
<<<<<<< .mine
||||||| .r3220
include_once(dirname(__FILE__) . "../../../../../modelos/dao/SelectDAO.php");
=======
...
class ProyectosProgramaticosController {
    ...
    public function getProyectosProgramaticosDataTable($params)
    {
        ...
        $sql = array(
            ...
            "tablas" => 
                "tblproyectosprogramaticos as PP, tblestatus as ESP, tblestatus as ESF",
            "orders" => $params["order"]["column"] . " " . $params["order"]["dir"],
            "where" => 
                ' PP.cveEstatusFinanzas = ESF.cveEstatus AND PP.cveEstatusPlaneacion = ESP.cveEstatus AND PP.activo = "S" '
        );
    ...
    public function loadProyectoProgramatico($idProyectoProgramatico)
    {
        ...
        $sql = array(
            "campos" => 
                "PP.idProyectoProgramatico, PP.cvePdeIdeal, PP.cvePdeEstrategia, PP.cvePdeLineaAccion, PP.anioProyectoProgramatico, PP.fechaInicio, PP.fechaTermino, PP.desProyectoProgramatico, PP.objetivo, PP.numEmpleadoResponsable, PP.estrategiaProyecto, PP.metaProyecto",
            "tablas" => 
    ...
    public function getUnidadesEjecutorasHasProyectoProgramaticoByIdProyectoPresupuestal($idProyectoPresupuestal)
    {
        ...
        $sql = array(
            ...
            "where" => 
                ' UE.idProyectoPresupuestal = '.$idProyectoPresupuestal.' AND UE.cveAdscripcion = PA.cveAdscripcion AND PA.idProyectoProgramatico = PP.idProyectoProgramatico AND PP.cveEstatusFinanzas = 29 AND PP.cveEstatusPlaneacion = 23 AND UE.activo = "S" AND PP.activo = "S" '
        );
        $sqlSelect = array("tabla" => "", "d" => $d, "tmpSql" => $sql, "proveedor" => null);
        $rs = $genericoDao->select($sqlSelect);
        return $rs;
