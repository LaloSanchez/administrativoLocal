...
include_once(dirname(__FILE__)."/../Skype.php");
include_once(dirname(__FILE__)."/../Skype/Bot/Plugin.php");
...
class Skype_Bot {
	...
	public function loadPlugin($plugin_id, $parameter) {
		...
		if (class_exists($klass) == false) {
			$path = sprintf(dirname(__FILE__)."/../Skype/Bot/Plugin/%s.php", ucfirst(strtolower($plugin_id)));
			include_once($path);
		}
		if (class_exists($klass) == false) {
