...
fielnet.Firma.prototype = (function () {
    ...
    function getVectorFile(strDigest, iCodification, strName, fCallback) {
        signPKCS1OffLine(strDigest, fielnet.Digest.SHA2, fielnet.Digest.B64, function (e) { //Firma digestión archivo
            signPKCS1OffLine(codigo, fielnet.Digest.SHA2, fielnet.Digest.UTF8, function (firmaCodigoResult) { //Firma código de activación
                var strCN = getCertificateAttributeFromSubject(getCertificate(), "2.5.4.3");
                ajaxRequest({
                    url: CONTROLLER,
                    data: "metodo=firmaArchivo" + "&referencia=" + strReferencia + "&digestion=" + strDigest + "&firma=" + e.sign + "&codigo=" + codigo + "&transferencia=" + transferencia + "&cert=" + getCertificate() + "&firmaCodigo=" + firmaCodigoResult.sign + "&serie=" + getSerialNumber(getCertificate()) + "&cn=" + strCN + "&fileName=" + strName + extraParameters,
                    method: "POST",
                    success: function (oResponse, status, xmlhttp) {
                        if (typeof fCallback == "function") {
                            var oJSONResponse = JSON.parse(oResponse);
                            oJSONResponse.sign = e.sign;
    ...
    function getCodeAndTransfer(fCallback) {
        ajaxRequest({
            url: CONTROLLER,
            method: "POST",
            data: "metodo=getCodeAndTransfer&curp=" + getCertificateAttributeFromSubject(getCertificate(), "2.5.4.5"),
            success: function (oResponse, status, xmlhttp) {
    ...
    return {
                ...
                if (result.state == 0) {
                    ...
                    for (var i = 0; i < files.length; i++) {
                        getFileDigest(files[i], iChunkSize, iAlgoritm, function (nombreArchivo, digestion) {
                            signPKCS1OffLine(digestion, iAlgoritm, fielnet.Encoding.B64, function (resultadoFirmaArchivo) {
                                var archivoFirmado = {};
                    ...
                    var iInterval = setInterval(function () {
                        if (totalArchivosFirmados == files.length) {
                            ...
                            signPKCS1OffLine(codigo, iAlgoritm, fielnet.Digest.UTF8, function (firmaCodigoResult) { //Firma código de activación
                                var tipoDigestion = iAlgoritm == fielnet.Digest.MD5 ? "MD5" : iAlgoritm == fielnet.Digest.SHA1 ? "SHA1" : iAlgoritm == fielnet.Digest.SHA2 ? "SHA256" : "SHA1";
                                var strCN = getCertificateAttributeFromSubject(getCertificate(), "2.5.4.3");
                                ajaxRequest({
                                    url: CONTROLLER,
         ...
         */
        signFileDigest: function (strId, iAlgoritm, fCallback) {
            var oResult = {};
            if (oPrivateKey == null) {
                if (typeof fCallback == "function") {
                    oResult.state = -78;
            ...
            if (oElement) {
                try {
                    var digestionesArchivos = JSON.parse(oElement.value);
                    var digestiones = digestionesArchivos.digestiones;
                    if (digestiones) {
                        getCodeAndTransfer(function (result) {
                            if (result.state == 0) {
                                var archivosFirmados = [];
                                for (var idx in digestiones) {
                                    var documentoFirma = digestiones[idx];
                                    var nombreDocumento = documentoFirma.documento;
                                    var digestionDocumento = documentoFirma.digestion;
                                    if (!base64regex.test(digestionDocumento)) {
                                        console.log("Digestión no válida: " + digestionDocumento);
                                        continue;
                                    }
                                    signPKCS1OffLine(digestionDocumento, iAlgoritm, fielnet.Digest.B64, function (resultadoFirmaArchivo) {
                                        var archivoFirmado = {};
                                        archivoFirmado.nombre = nombreDocumento;
                                        archivoFirmado.digestion = digestionDocumento;
                                        archivoFirmado.firma = resultadoFirmaArchivo.sign;
                                        archivosFirmados.push(archivoFirmado);
                                ...
                                signPKCS1OffLine(codigo, iAlgoritm, fielnet.Digest.UTF8, function (firmaCodigoResult) { //Firma código de activación
                                    ...
                                    ajaxRequest({
                                        url: CONTROLLER,
                                        data: "metodo=firmaArchivo&codigo=" + codigo + "&transferencia=" + transferencia + "&cert=" + getCertificate() + "&firmaCodigo=" + firmaCodigoResult.sign + "&serie=" + getSerialNumber(getCertificate()) + "&cn=" + strCN + "&jsonData=" + JSON.stringify(archivosFirmados) + "&tipoDigestion=" + tipoDigestion + extraParameters,
                                        method: "POST",
                                        success: function (oResponse, status, xmlhttp) {
                                            if (typeof fCallback == "function") {
                                                var oJSONResponse = JSON.parse(oResponse);
                                                oResult = oJSONResponse;
                                            }
                                        }
                                    });
                                });
                            } else {
                                oResult.state = result.state;
                                oResult.description = result.description;
                            }
                        });
                    } else {
                        oResult.state = -75;
