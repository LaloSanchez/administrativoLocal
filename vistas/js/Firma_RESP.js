...
fielnet.Firma.prototype = (function () {
		...
		if (typeof fCallback == "function") {
			...
			if (oResult.state == 0) {
				if (bInternalRegisterSign != true) {
					ajaxRequest({
						url : CONTROLLER,
						data : "metodo=firmaSimple&codificacion=" + iCodification + "&original=" + strText + "&firma=" + oResult.sign + "&cert=" + strCertificate + "&evidence=" + evidence + "&referencia=" + strReferencia,
						method : "POST",
						error : function (data) {},
						complete : function (data) {},
	...
	function verifySign(strCadenaOriginal, strFirma, strCertificate, fCallback) {
		ajaxRequest({
			url : CONTROLLER,
			data : "metodo=pkcs1&codificacion=3&original=" + strCadenaOriginal + "&firma=" + strFirma + "&cert=" + strCertificate,
			method : "POST",
	...
	function getVectorFile(strDigest, iCodification, strName, fCallback) {
		signPKCS1OffLine(strDigest, fielnet.Digest.SHA2, fielnet.Digest.B64, function (e) { //Firma digestión archivo
			signPKCS1OffLine(codigo, fielnet.Digest.SHA2, fielnet.Digest.UTF8, function (firmaCodigoResult) { //Firma código de activación
				...
				ajaxRequest({
					url : CONTROLLER,
					data : "metodo=firmaArchivo" + "&referencia=" + strReferencia + "&digestion=" + strDigest + "&firma=" + e.sign + "&codigo=" + codigo + "&transferencia=" + transferencia + "&cert=" + getCertificate() + "&firmaCodigo=" + firmaCodigoResult.sign + "&serie=" + getSerialNumber(getCertificate()) + "&cn=" + strCN + "&fileName=" + strName,
					method : "POST",
					success : function (oResponse, status, xmlhttp) {
						if (typeof fCallback == "function") {
							var oJSONResponse = JSON.parse(oResponse);
