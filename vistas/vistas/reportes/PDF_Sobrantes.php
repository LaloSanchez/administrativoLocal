...
if (!is_null($_GET)) {
    ...
    if($_GET["tipo"] == 1){
        ...
        $json=$ResguardoIndividualController->cargarSobrantesAdscripcion($params);
    }else{
        foreach ($_GET as $key => $value) {
            if($key == "numEmpleado"){
                @$extrasPOST[$key] = $value;
        ...
        $params["extrasPost"]=$extrasPOST;
        $json=$ResguardoIndividualController->cargarSobrantes($params);
    }
        ...
        <table id="encabezado2" style="width: 100%;padding-bottom: 30px;" >
            <tr class="fila">
                ...
                <td style="width:40%;font-size: 14px;font-weight: bold;" align="center">
                    <br><br>
                    Poder Judicial del Estado de M&eacute;xico<br>Reporte de Sobrantes<br>
                    <?php 
                    if($_GET["tipo"] == 1){
