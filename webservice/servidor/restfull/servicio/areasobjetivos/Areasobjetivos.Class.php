...
class Areasobjetivos {
    
    public function consultarObjetivos($cveAdscripcion){
        $genericoDao = new GenericDAO();      
        $arrayObjetivos=array();
        $d = array("limit" => "");        
        $sql = array("campos" => "desAreaObjetivo",
            "tablas" => "tblareaobjetivos",
        ...
        if($data["totalCount"]>0){
            for ($i=0; $i < $data["totalCount"] ; $i++) {
                $arrayObjetivos[]=$data["data"][$i]["desAreaObjetivo"];
            }
